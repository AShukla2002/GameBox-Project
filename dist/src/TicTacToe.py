...
icon_path = pygame.image.load('data/ttt.ico')
...
def drawxo():
    for row in range(3):
        for col in range(3):
            if board[row][col] == 1:
                pygame.draw.circle(
                    root, ocolor, (int(col*size+size//2), int(row*size+size//2)), rad, owidth)
            elif board[row][col] == 2:
                pygame.draw.line(root, xcolor, (col*size+gap, row*size +
                                 size-gap), (col*size+size-gap, row*size+gap), xwidth)
                pygame.draw.line(root, xcolor, (col*size+gap, row*size+gap),
                                 (col*size+size-gap, row*size+size-gap), xwidth)
